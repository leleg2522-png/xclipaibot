...
- Users buy saldo themselves via `/topup` → type the number of videos → receive a QRIS image. On payment, saldo is credited **automatically**.
- Provider: **KlikQRIS "PG KlikQRIS" (`/api/qris`)**, base `https://klikqris.com/api/qris`. Auth via headers `x-api-key` + `id_merchant`. (Migrated from the older `/api/qrisv2`, which had NO self-service webhook — `/api/qris` supports a per-transaction `callback_url`.)
- Create: `POST /create` `{ order_id, id_merchant, amount, keterangan, callback_url }` → `data.total_amount` (charge this to the buyer; includes a unique-code fee so it must be paid EXACTLY), `data.qris_url` (QR image), `data.signature`, `data.expired_at`, `status:"PENDING"`. No per-order `direct_url` in this API (users pay by scanning the QR image). `callback_url` is set automatically to `https://<PUBLIC_DOMAIN>/webhook/klikqris`.
- Status: `GET /status/{order_id}` (order_id only in path; merchant is passed via headers). Returns `status` of `PENDING` | `SUCCESS` (= paid) | `EXPIRED`. **Note the paid state is `SUCCESS` on the status endpoint but `PAID` in the webhook payload — code treats both as paid.**
- **Webhook** `POST /webhook/klikqris` is now the primary accelerator: `/api/qris` POSTs to the per-transaction `callback_url` on `PAID`/`EXPIRED`. Its payload is **FLAT** (`order_id`/`status`/`signature` at the root), unlike the old nested `body.data`; the handler supports both. Verified by matching the webhook `signature` against the signature stored at create time (NOT HMAC), idempotent, always returns HTTP 200.
- **Crediting is also NOT webhook-dependent** (defense in depth): a background **auto-poll** sweep (`startTopupPoller`/`sweepPendingTopups`, every `TOPUP_POLL_INTERVAL_MS`≈15s) checks each recent `PENDING` order via `/status` and credits automatically on `SUCCESS`. Reads from the DB so it survives restarts; only polls orders newer than `TOPUP_POLL_WINDOW_MINUTES` (120) to bound load, and **never force-expires by age** (that could block a legitimate late payment) — an order only becomes `EXPIRED` when the provider itself reports it. Concurrency-limited (`TOPUP_POLL_CONCURRENCY`). The manual "Cek pembayaran" button also polls `/status`.
...
- **No packages, no preset buttons.** Price is flat: `PRICE_PER_VIDEO` (Rp2.000) per video (`index.js`). Charge = `jumlah × PRICE_PER_VIDEO`. Users either type `/topup <jumlah>` (e.g. `/topup 7`) or send `/topup` alone → bot sets `session.awaitingTopupQty` and the next plain-number message is taken as the quantity (handled in the `bot.on("text")` handler). Min/max from `TOPUP_MIN_VIDEOS`/`TOPUP_MAX_VIDEOS` (1–500).
